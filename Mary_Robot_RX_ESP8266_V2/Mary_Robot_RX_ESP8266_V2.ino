...
void setup() {
  ...
  esp_now_register_recv_cb([](uint8_t *mac, uint8_t *incomingData, uint8_t len) {
    ...
    if (strcmp(msg, "B") == 0) moveForward();
else if (strcmp(msg, "F") == 0) moveBackward();
else if (strcmp(msg, "R") == 0) turnLeft();
else if (strcmp(msg, "L") == 0) turnRight();
else stopMotors();
...
// Move Forward
void moveForward() {
  analogWrite(ENA, 250); // speed 0-255
  analogWrite(ENB, 250);
  digitalWrite(L1, LOW);
